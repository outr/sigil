...
- The **agent self-loop** — `tryFire` claims an `AgentState(Active)` lock via `tx.modify` on a deterministic lock id (`agentlock:<agentId>:<convId>`). The winner runs `runAgentLoop` on a background fiber, checking for new triggers between iterations without releasing the claim until settled. `maxAgentIterations` (default 10) hard-caps runaways with `AgentRunawayException`. **The lock is per-process** (RocksDB / Postgres local); multi-replica deployments need app-side routing (sticky sessions to a single Sigil instance per conversation) — Sigil does not ship a distributed agent-claim primitive. Apps that want active/active need to either pin conversations to instances at the load balancer or implement their own claim sweep on top of `SigilDB.events`.

- **Stop semantics** — a `Stop` event sets the matching `StopFlag` so the agent loop's next iteration-boundary check exits. The in-flight HTTP stream runs to natural completion (single-digit seconds typically) before the loop's `stopFlag.exists(_.requested)` check fires; the data from the in-flight call is settled normally and just not used to drive a next iteration. **True mid-stream HTTP abort is not shipped** — that would require spice's `HttpClient` to expose a cancellation handle, which it currently doesn't. Apps that need sub-second cancellation today do it at their HTTP layer (e.g., closing the SSE connection externally).
- **Polymorphic type registration** — `signalRegistrations`, `participants`, `participantIds`, `spaceIds`, plus `findTools.toolInputRWs`. These feed fabric `PolyType.register` at `instance` initialization. You MUST list app-specific subtypes here or fabric RW will fail to round-trip them.
...
`Provider.models: List[Model]` reads from `Sigil.cache: ModelRegistry` — an in-memory `AtomicReference[Map[Id[Model], Model]]` populated by `OpenRouter.refreshModels` and persisted to `${dbPath}/models.json` as a fallback for offline boots. Reads are synchronous (no DB roundtrip), safe to call on hot paths like `isImageOnlyModel`. `Sigil.modelRefreshInterval: Option[FiniteDuration]` defaults to `None` — apps that don't use OpenRouter-sourced models (local llama.cpp, direct-Anthropic, direct-OpenAI) get no background HTTP traffic. OpenRouter consumers override to e.g. `Some(8.hours)` for periodic refresh, or leave `None` and call `OpenRouter.refreshModels` manually.
...
- **`RateLimiter`** (`Provider.rateLimiter`, default `RateLimiter.NoOp`) — proactive pacing. The framework's `apply` calls `acquire` before each outgoing request and `Task.sleep`s when the bucket falls below configured floors (default `softFloor = 5%` → 250ms, `hardFloor = 1%` → 1s). Apps that want pacing wire `observe(remainingRequests, remainingTokens, resetSeconds, retryAfter)` themselves — spice's `streamLines()` doesn't surface response headers, so the framework can't auto-feed `observe()` from a provider's response. Typical pattern: app fronts the provider with its own HTTP layer (or a wrapper) that tees rate-limit headers (`x-ratelimit-remaining-*`, `retry-after`) into the limiter. `RateLimiter.forKey(apiKey)` returns a per-API-key shared limiter so two provider instances against the same upstream account share one limiter. Distinct from `ProviderStrategy`'s reactive cooldown — the strategy decides what to do AFTER a 429; the rate limiter tries to stop the 429 from happening, when the app feeds it data.
...
`Tool[Input <: ToolInput]` is case-class-derived: `Input derives RW` → JSON schema is generated by `DefinitionToSchema`. Core tools live in `sigil/tool/core/` (`RespondTool`, `ChangeModeTool`, `FindCapabilityTool`, `StopTool`, `NoResponseTool`). Apps register app-specific tools by appending to `Sigil.staticTools` (the [[StaticToolSyncUpgrade]] upserts those into `SigilDB.tools` on every startup) or by persisting per-record entries via `Sigil.createTool`. `Sigil.findTools` defaults to a [[DbToolFinder]] over the persisted collection; apps override only when they need a custom catalog (marketplace union, in-memory test fixture, etc.). `ConsultTool.invoke` runs a focused one-shot tool call (e.g. `TopicClassifierTool`, `ExtractMemoriesTool`) bypassing the conversation loop.
...
Three pathways, all writing `ContextMemory` with a `SpaceId` discriminator:
...
**Memory-write metadata.** Beyond the core fact / source / spaceId / key tuple, `ContextMemory` carries optional governance fields. **Important:** these are app-side metadata only — the framework persists them but does NOT consume them in ranking or eviction. Apps that care wire their own retrieval ranker / sweep effect.

- `confidence: Double` (default 1.0) — how sure the writer is the fact is correct. The default `StandardMemoryRetriever` does NOT rank by confidence; apps that want confidence-aware ordering provide their own `MemoryRetriever` (or pre-filter results before passing to the curator).
- `expiresAt: Option[Timestamp]` — retrieval-time TTL filter. `StandardMemoryRetriever.isExpired(m, now)` skips records whose `expiresAt` is set and not in the future, so an expired Critical memory stops being rendered without losing the durable record. **The store row persists indefinitely — there is no automatic eviction sweep.** Apps that want hard eviction (DB row deletion) wire their own `SettledEffect` that periodically queries `expiresAt < now` and `tx.delete(_id)`. Distinct from versioning's `validUntil` (which marks superseded versions, not TTL-evicted ones).
- `justification: Option[String]` — free-form reason the memory was recorded ("inferred from explicit user request 2026-05-02"). Audit / debugging signal only; the framework never surfaces it in agent context. Apps display in pinned-memory review UIs.
...
Discovery filtering — what `find_capability` returns — runs in two stages. `DiscoveryFilter.passesAffinity(tool, request)` (mode + space affinity) is applied inside the `ToolFinder` so DB-backed finders push the constraint into the index. `DiscoveryFilter.passesPolicy(tool, mode.tools)` is applied by `Sigil.findCapabilities` after the finder returns, so `Exclusive(names)` / `Scoped(names)` truly restrict the discovery catalog (the docstring claim) without each finder having to know about ToolPolicy. Apps with cross-mode-gating beyond the standard policies swap the whole finder by overriding `Sigil.findTools(request)` and / or override `findCapabilities` for source-fanout. The framework deliberately keeps the path predicate-based, not list-based — `ToolFinder` produces keyword-matched subsets (DB-backed finders may stream) and the predicates are applied per result. No "all tools" list ever materializes.
