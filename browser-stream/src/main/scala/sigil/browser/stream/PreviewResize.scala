 ...
 * session rebuilds its pipeline and renegotiates, a screencast re-lays
 * the page out and restarts capture behind the same frame stream — so
 * [[StreamBrowserSigil.resizePreview]] holds this rather than a session
 * kind.
 */
trait PreviewResize {
