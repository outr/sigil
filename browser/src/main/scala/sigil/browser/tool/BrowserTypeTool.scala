...
final class BrowserTypeTool extends Tool {
  ...
  val io: ToolIO[BrowserTypeInput, TextToolOutput] = ToolIO.derived[BrowserTypeInput, TextToolOutput].withExamples(
  ...
  private def executeResult(input: BrowserTypeInput, ctx: ToolContext): Task[ToolResult[TextToolOutput]] =
    for {
