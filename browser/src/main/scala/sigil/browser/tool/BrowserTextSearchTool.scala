...
final class BrowserTextSearchTool extends Tool {
  ...
  val io: ToolIO[BrowserTextSearchInput, TextToolOutput] = ToolIO.derived[BrowserTextSearchInput, TextToolOutput].withExamples(
    ToolExample(
  ...
  private def executeResult(input: BrowserTextSearchInput, ctx: ToolContext): Task[ToolResult[TextToolOutput]] =
