...
final class BrowserClickTool extends Tool {
  ...
  val io: ToolIO[BrowserClickInput, TextToolOutput] = ToolIO.derived[BrowserClickInput, TextToolOutput].withExamples(
  ...
  private def executeResult(input: BrowserClickInput, ctx: ToolContext): Task[ToolResult[TextToolOutput]] =
    for {
