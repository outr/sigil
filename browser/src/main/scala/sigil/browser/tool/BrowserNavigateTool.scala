...
final class BrowserNavigateTool extends Tool {
  ...
  val io: ToolIO[BrowserNavigateInput, TextToolOutput] = ToolIO.derived[BrowserNavigateInput, TextToolOutput].withExamples(
    ToolExample("Open a homepage", BrowserNavigateInput(url = "https://example.com/")),
    ToolExample("Open with a longer wait for slow pages", BrowserNavigateInput(url = "https://news.example/", waitForLoadSeconds = 30))
  )
  ...
  private def executeResult(input: BrowserNavigateInput, ctx: ToolContext): Task[ToolResult[TextToolOutput]] =
  ...
  private def doNavigate(input: BrowserNavigateInput, ctx: ToolContext): Task[ToolResult[TextToolOutput]] =
    for {
