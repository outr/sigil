...
case class BrowserScript(override val name: ToolName,
   ...
   */
  val io: ToolIO[JsonInput, TextToolOutput] = ToolIO.dynamic(parameters)

  protected def resolve: Resolution[Input, Output] =
    Resolution.Explicit((input, context) => BrowserScript.runSteps(this, input.json, context))
}
...
object BrowserScript {
   ...
   */
  private[browser] def runSteps(script: BrowserScript, args: Json, context: ToolContext): Task[ToolResult[TextToolOutput]] =
    context.sigil match {
