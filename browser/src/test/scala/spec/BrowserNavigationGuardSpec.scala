...
class BrowserNavigationGuardSpec extends AnyWordSpec with Matchers {
  ...
  private def navigate(url: String): ToolResult[?] = {
    ...
    tool.invoke(BrowserNavigateInput(url = url), ToolContext(turn, Event.id(), tool.name))
      .map(out => ToolResult.Success(out): ToolResult[?])
      .handleError {
        case tfe: sigil.tool.ToolFailureException =>
          rapid.Task.pure(ToolResult.Failure(tfe.failureMessage, tfe.hint, tfe.args): ToolResult[?])
        case other => rapid.Task.error(other)
      }.sync()
  }
