...
class LspRenameSymbolSpec extends AsyncWordSpec with AsyncTaskSpec with Matchers {
  ...
  implicit override val testTimeout: FiniteDuration = 10.minutes
  ...
  private def runScenario(layout: ScalaWorkspaceLayout,
                          indexProbe: String,
                          minIndexMatches: Int,
                          input: Path => LspRenameSymbolInput)(assertions: (Path, LspRenameSymbolOutput) => org.scalatest.Assertion)
    : Task[org.scalatest.Assertion] = {
 ...
 */
sealed private trait ScalaWorkspaceLayout
  ...
  def materialize(layout: ScalaWorkspaceLayout): Path = {
    ...
    layout match {
      ...
      case ScalaWorkspaceLayout.ClassAndMethod(name) =>
        ...
        write(
          srcDir.resolve("Container.scala"),
          s"""object Container {
             |  def $name(): Unit = ()
             |}
             |""".stripMargin
