...
case class LspTypeDefinitionInput(languageId: String, filePath: String, line: Int, character: Int) extends ToolInput derives RW
 ...
 */
final class LspTypeDefinitionTool(val manager: LspManager) extends Tool with LspToolSupport {
  type Input = LspTypeDefinitionInput
  type Output = LspLocationsResult
  val io: ToolIO[LspTypeDefinitionInput, LspLocationsResult] = ToolIO.derived[LspTypeDefinitionInput, LspLocationsResult]
  ...
  private def executeOutput(input: LspTypeDefinitionInput, context: ToolContext): Task[LspLocationsResult] =
