...
case class LspWorkspaceSymbolsInput(languageId: String, projectRoot: String, query: String, maxResults: Int = 100) extends ToolInput
  derives RW
 ...
 */
final class LspWorkspaceSymbolsTool(val manager: LspManager) extends Tool with LspToolSupport {
  type Input = LspWorkspaceSymbolsInput
  type Output = TextToolOutput
  val io: ToolIO[LspWorkspaceSymbolsInput, TextToolOutput] = ToolIO.derived[LspWorkspaceSymbolsInput, TextToolOutput]
  ...
  private def executeResult(input: LspWorkspaceSymbolsInput, context: ToolContext): Task[ToolResult[TextToolOutput]] =
