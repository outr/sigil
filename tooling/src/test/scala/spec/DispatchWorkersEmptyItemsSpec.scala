...
import sigil.tooling.dispatch.{
  ...
  LlmStep, WorkerItemSource, WorkerPipeline
}
 ...
 * `items` resolves to an empty list AND `confirmed: true`, the tool
 * returns a structured failure carrying a "nothing to dispatch" hint
 * rather than silently completing as if work happened. When
 * `confirmed: false` the same input returns a scope preview with
 * zero workers — no rejection.
 */
  ...
  "dispatch_workers" should {
    ...
    "reject empty items + confirmed=true with a structured 'nothing to dispatch' failure" in {
      ...
      val input = DispatchWorkersInput(
        ...
        confirmed  = true,
        items      = WorkerItemSource.FromList(Nil),
