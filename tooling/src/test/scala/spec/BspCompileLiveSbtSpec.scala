...
class BspCompileLiveSbtSpec extends AsyncWordSpec with AsyncTaskSpec with Matchers {
  ...
  implicit override protected val testTimeout: FiniteDuration = 10.minutes
  ...
  private def writeSources(dir: Path, broken: Boolean): Unit =
    if (broken) {
      Files.writeString(
        dir.resolve("src/main/scala/Bad1.scala"),
        "object Bad1 { def broken(: Int = 1 }\n")
      Files.writeString(
        dir.resolve("src/main/scala/Bad2.scala"),
        "object Bad2 { val x = \n")
    } else {
      Files.writeString(
        dir.resolve("src/main/scala/Bad1.scala"),
  ...
  private def deleteRecursively(dir: Path): Unit = {
    ...
    try Files.walk(dir).iterator().asScala.toList.reverse.foreach(p =>
        try Files.delete(p)
  ...
  "bsp_compile against a real sbt BSP server" should {
    ...
    "surface per-file diagnostics for a broken workspace, then OK once fixed" in {
      ...
      sigil.instance.flatMap { _ =>
        ...
        val cleanup = Task {
          try manager.shutdown(root).sync()
          catch { case _: Throwable => () }
