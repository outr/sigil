...
case class DapSetExceptionBreakpointsInput(sessionId: String, filters: List[String]) extends ToolInput derives RW
...
final class DapSetExceptionBreakpointsTool(val manager: DapManager) extends Tool with DapToolSupport {
  ...
  val io: ToolIO[DapSetExceptionBreakpointsInput, TextToolOutput] =
    ToolIO.derived[DapSetExceptionBreakpointsInput, TextToolOutput].withExamples(
      ToolExample(
  ...
  private def executeResult(input: DapSetExceptionBreakpointsInput, context: ToolContext): Task[ToolResult[TextToolOutput]] =
