...
import sigil.tool.{
  DiscoverySpec, Effect, Freshness, PlaceholderInputDetector, Resolution, TextToolOutput, Tool, ToolExample, ToolIO, ToolName, ToolProfile,
  ToolResult, ToolSpec
}
...
final class GrepTool(context: FileSystemContext) extends Tool {
  ...
  val spec: ToolSpec = ToolSpec(
    name = ToolName("grep"),
    description = """Search files under `path` for a regex `pattern`. Output is bounded and lands directly in
