 ...
 * unaffected.
 */
case class WorkflowStepSpec(
  @description("Unique id for this step within the workflow. Other steps reference it (a Loop's bodyStepIds, a Condition's onTrue/onFalse, a Parallel's branchStepIds).")
  id: String,
  ...
  @description(
    "Loop only: the ids of the steps (defined elsewhere in this same flat list) to run once per item. Required and non-empty for a Loop.")
  ...
  trigger: Option[WorkflowTrigger] = None)
  derives RW
  ...
  private def build(s: WorkflowStepSpec, byId: Map[String, WorkflowStepSpec]): WorkflowStepInput = s.kind match {
    case Job =>
      ...
      JobStepInput(
        ...
        s.prompt,
        s.tool,
        s.arguments.map(JsonFormatter.Compact(_)),
        s.output,
        s.complexity,
        s.tools,
        s.continueOnError,
    ...
    case Approval =>
      ApprovalStepInput(
        ...
        s.prompt.get,
        s.name,
        if (s.options.nonEmpty) s.options else List("approve", "reject"),
        s.output,
        s.timeoutMs,
        s.timeoutAction.getOrElse(TimeoutAction.Fail)
      )
    case Parallel =>
      ParallelStepInput(
        s.id,
        s.branchStepIds.map(_.flatMap(byId.get).map(build(_, byId))),
        s.name,
        s.joinMode.getOrElse(JoinMode.All),
        s.output)
    case Loop =>
      LoopStepInput(
        s.id,
        s.over.get,
        s.bodyStepIds.flatMap(byId.get).map(build(_, byId)),
        s.name,
        s.itemVariable.getOrElse("item"),
        s.output)
    case SubWorkflow =>
