...
object WorkflowBuilderSkill {
  val text: String =
    """You are in WORKFLOW BUILDER mode. Your job is composing, editing, and running typed workflows on top of Sigil's `sigil-workflow` runtime.
      ...
      |
      |  - kind = "Job" — runs a tool or an LLM prompt. Set `tool` + `arguments` (a JSON OBJECT holding every tool parameter — not a string) to invoke a tool, OR `prompt` to run an LLM prompt. A prompt step's model is selected AUTOMATICALLY from the conversation's mode + an optional `complexity` tier (Low | Medium | High | VeryHigh) — you never name a model; omit `complexity` to auto-classify, or set it to bias an easy/hard leaf. `output` names the variable the result is written to. `tools` (optional) restricts an LLM step's tool roster.
