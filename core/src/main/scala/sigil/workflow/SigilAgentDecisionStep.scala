...
final case class SigilAgentDecisionStep(input: AgentDecisionStepInput,
      ...
      host.providerFor(modelId, Nil).flatMap { provider =>
        ...
        provider(request).evalMap {
          ...
          toolCompletionRef.get() match {
            ...
            case Some(summary) =>
              Task.pure(SigilAgentDecisionStep.withUsage(
                obj(
                  "complete" -> bool(true),
                  "summary" -> str(summary),
  ...
  private def decideNext(host: sigil.Sigil,
                         workflow: Workflow,
                         response: String,
                         usage: Option[TokenUsage],
                         ctx: Option[JobContext]): Task[Json] =
    SigilAgentDecisionStep.parseMarker(response) match {
      ...
      case SigilAgentDecisionStep.MarkerAskParent(question) =>
        suspendForAnswer(host, workflow, response, question, ctx).map { questionId =>
          SigilAgentDecisionStep.withUsage(
            obj(
              "complete" -> bool(false),
              "asked" -> bool(true),
   ...
   */
  private def suspendForAnswer(host: sigil.Sigil,
                               workflow: Workflow,
                               response: String,
                               question: String,
                               ctx: Option[JobContext]): Task[String] = host match {
    case ws: WorkflowSigil =>
      val workerConvIdOpt = workflow.conversationId.map(s => Id[Conversation](s))
  ...
  private def publishToParent(host: sigil.Sigil, workerConvId: Id[Conversation], questionId: String, question: String): Task[Unit] =
    host.withDB(_.conversations.transaction(_.get(workerConvId))).flatMap {
      case Some(workerConv) => workerConv.parentConversationId match {
            ...
            host.withDB(_.conversations.transaction(_.get(parentConvId))).flatMap {
              case Some(parentConv) =>
                parentConv.participants.headOption match {
                  ...
                  case None =>
                    Task {
                      scribe.warn(s"AgentDecisionStep ask_parent: parent conv $parentConvId has no participants; question dropped"); ()
                    }
                }
              case None =>
          ...
          case None =>
            Task {
              scribe.warn(s"AgentDecisionStep ask_parent: worker conv $workerConvId has no parentConversationId; question dropped"); ()
            }
        }
      case None =>
  ...
  private def appendNextIteration(host: sigil.Sigil, workflow: Workflow, lastResponse: String, ctx: Option[JobContext]): Task[Unit] =
    host match {
      case ws: WorkflowSigil =>
