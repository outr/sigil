...
trait AgentLoopOps { this: Sigil =>
   ...
   */
  final private[sigil] class ActiveClaimEntry(@volatile var lastTriggerCheckAt: Long,
  ...
  final private[sigil] val activeClaims: ConcurrentHashMap[Id[Event], ActiveClaimEntry] = new ConcurrentHashMap()
   ...
   */
  final private val pendingTriggers: ConcurrentHashMap[Id[Event], java.lang.Long] = new ConcurrentHashMap()
   ...
   * re-fire from it.
   */
  final private[sigil] def clearPendingTriggers(conversationId: Id[Conversation]): Unit = {
   ...
   */
  final private def spendPendingTrigger(lockId: Id[Event], cutoff: Long): Unit =
    pendingTriggers.computeIfPresent(
      lockId,
      (_, recordedAt) =>
   ...
   */
  final private[sigil] def recordTurnEvent(e: Event): Unit =
   ...
   * log.
   */
  final private[sigil] def applyStop(signal: Signal): Task[Unit] = signal match {
    case s: Stop =>
      val setFlags = Task {
  ...
  final private[sigil] def fanOut(event: Event): Task[Unit] = {
   ...
   */
  final private def recordPendingTrigger(agent: AgentParticipant,
                                         conv: Conversation,
                                         lockId: Id[Event],
  ...
  final private def tryFire(agent: AgentParticipant,
                            conv: Conversation,
   ...
   */
  final private[sigil] def runAgentLoop(agent: AgentParticipant,
...
//
                  ...
                  val drainTask =
                    interruptible
                      .evalTap {
                        ...
                        case ti: ToolInvoke
                            if ti.toolName.value == Directive.RefusalChallengeName
                        ...
                        case td: ToolDelta
                          ...
                          Task {
                            ...
                            val keepsTurnOpen = toolName == "respond" &&
                              (td.input match {
   ...
   * there.
   */
  final private def fireReplySuggestions(agent: AgentParticipant,
                                         convId: Id[Conversation],
  ...
  final private def suggestRepliesFor(agent: AgentParticipant,
                                      conv: Conversation,
  ...
  final private def runReplySuggestionConsult(agent: AgentParticipant,
                                              config: ReplySuggestionsConfig,
   ...
   */
  final private def isExternalTrigger(agent: AgentParticipant, e: Event): Boolean =
   ...
   */
  final private[sigil] def agentStateLockId(agentId: AgentParticipantId, convId: Id[Conversation]): Id[Event] =
