...
object Instructions {
  ...
  val DefaultToolsGuidance: String =
    """TOOLS — discovery-first is the framework's CORE ideology. Internalize this.
      ...
      |   → **STEP A: ONLY IF STEP 0 left you in a mode that still doesn't fit (i.e., you're in 'conversation' but the user is clearly starting a sustained session in a more specific mode's domain),** check the available-modes list (in `change_mode`'s description below). Call `change_mode("<name>")` ONLY when the user is starting a sustained session in that mode's domain (multi-turn work the mode is designed for). Each mode's description spells out "Enter when…" and "Don't enter for…" — honor those. A single-action request ("connect my project", "open this file", "what does X mean") is NOT a mode switch; skip to STEP B and let find_capability surface the right tool.
      |   → **STEP B: when no mode applies (or for single-action requests),** call `find_capability` with keywords describing the task. Most tools (filesystem, LSP, BSP, memory, web fetch, MCP, set_workspace) are universally discoverable regardless of the active mode.
