...
trait PublishOps { this: Sigil =>
   ...
   */
  final private[sigil] lazy val hub: SignalHub = new SignalHub(signalHubCapacity)
   ...
   * projection.
   */
  final private def inlineContextFrame(event: Event): Event =
   ...
   * work to do.
   */
  final private def settlePairedToolFrame(written: Option[Event]): Task[Unit] = written match {
