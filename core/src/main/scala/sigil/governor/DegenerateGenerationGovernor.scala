 ...
 */
final class DegenerateGenerationGovernor(detector: DegenerateContentDetector = DegenerateContentDetector.Default) extends OutcomeGovernor {
        ...
        detector.detect(text) match {
          case Some(hit) =>
            ...
            OutcomeVerdict.Emit(SyntheticDiagnostic(
              Directive.DegenerateGeneration(
                hit.repeatedSentence,
                hit.occurrences,
                hit.totalSentences,
                hit.share,
                text.length),
              ...
              outcome.topicId,
              disposition = MessageDisposition.Failure(recoverable =
                true)
            ))
