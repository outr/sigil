...
class ConversationSelfHealSpec extends AsyncWordSpec with AsyncTaskSpec with Matchers {
  ...
  scribe.Logger.root
    .withHandler(writer = new scribe.writer.Writer {
      override def write(record: scribe.LogRecord,
   ...
   */
  final private class OrphanCallErrorProvider(callIdToCite: String) extends Provider {
   ...
   */
  final private class HealThenSucceedProvider(callIdToCite: String) extends Provider {
    val attemptCount = new atomic.AtomicInteger(0)
    override def `type`: ProviderType = ProviderType.LlamaCpp
    override def models: List[_root_.sigil.db.Model] = Nil
    override protected def sigil: _root_.sigil.Sigil = TestSigil
