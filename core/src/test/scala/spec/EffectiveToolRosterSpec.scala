...
class EffectiveToolRosterSpec extends AsyncWordSpec with AsyncTaskSpec with Matchers {
  ...
  private object ReadFileTool extends Tool {
    ...
    val io: ToolIO[ReadFileInput, ReadFileOutput] = ToolIO.derived[ReadFileInput, ReadFileOutput]
    ...
    private def executeResult(input: ReadFileInput, context: ToolContext): Task[ToolResult[ReadFileOutput]] =
  ...
  private class OutOfRosterProvider extends Provider {
    ...
    override def call(input: ProviderCall): Stream[ProviderEvent] = {
      ...
      Stream.emits(List(
        ...
        ProviderEvent.ToolCallComplete(cid, WireCall.Unresolved("read_file", fabric.obj("path" -> fabric.str("sections/main.liquid")))),
