...
class NarrowRosterByRecentUseSpec extends AsyncWordSpec with AsyncTaskSpec with Matchers {
  ...
  private val testModel: Model = {
    ...
    Model(
      ...
      topProvider = ModelTopProvider(contextLength = Some(200_000L), maxCompletionTokens = Some(2_048L), isModerated = false),
  ...
  private def stubTool(n: String): Tool { type Input = NarrowingStubInput } = new Tool {
    ...
    val io: ToolIO[NarrowingStubInput, TextToolOutput] = ToolIO.derived[NarrowingStubInput, TextToolOutput]
    ...
    private def executeOutput(input: NarrowingStubInput, ctx: ToolContext): Task[TextToolOutput] =
  ...
  private def callsFor(captured: ConcurrentLinkedQueue[ProviderCall], convId: Id[Conversation]): List[ProviderCall] =
   ...
   */
  private def waitForAgentIdle(convId: Id[Conversation], agentId: sigil.participant.ParticipantId, deadlineMs: Long): Task[Unit] = {
