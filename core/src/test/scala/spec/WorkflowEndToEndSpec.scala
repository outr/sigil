...
import sigil.provider.{
  CallId, ConversationMode, GenerationSettings, Instructions, Provider, ProviderCall, ProviderEvent, ProviderType, StopReason
}
...
class WorkflowEndToEndSpec extends AsyncWordSpec with AsyncTaskSpec with Matchers {
  ...
  "compose discovery as a workflow STAGE — a Loop consumes a discovery step's output, no hand-built array (#372 / workflow-first)" in {
    ...
    val template = WorkflowTemplate(
      ...
      steps = List(
        JobStepInput(
          id = "discover",
          tool = Some("discovery_probe"),
          arguments = Some("""{"count":3}"""),
          output = Some("found")),
        LoopStepInput(
          id = "loop",
          over = "found",
    ...
    val template = WorkflowTemplate(
      ...
      steps = List(
        JobStepInput(
          id = "discover",
          tool = Some("discovery_probe"),
          arguments = Some(s"""{"count":$bigCount}"""),
          output = Some("found")),
        LoopStepInput(
          id = "loop",
          over = "found",
  ...
  private def waitForTerminal(recorded: ConcurrentLinkedQueue[Signal], timeout: FiniteDuration): Task[Unit] = {
    ...
    def loop: Task[Unit] = Task.defer {
      import scala.jdk.CollectionConverters.*
      val seen = recorded.iterator().asScala.exists(s =>
        s.isInstanceOf[WorkflowRunCompleted] || s.isInstanceOf[WorkflowRunFailed])
      if (seen || System.currentTimeMillis() > deadline) Task.unit
      else Task.sleep(100.millis).flatMap(_ => loop)
    }
    loop
