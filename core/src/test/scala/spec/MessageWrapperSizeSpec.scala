...
class MessageWrapperSizeSpec extends AnyWordSpec with Matchers {
  ...
  "Provider.estimateMessage" should {
    ...
    "include the call id in the Assistant tool-call cost" in {
      val short = ProviderMessage.Assistant(content = "", toolCalls = List(ToolCallMessage(
        id = "x", name = "n", argsJson = "{}"
      )))
      val long = ProviderMessage.Assistant(content = "", toolCalls = List(ToolCallMessage(
        id = "x" * 200, name = "n", argsJson = "{}"
      )))
