...
class TurnContextSummarySpec extends AsyncWordSpec with AsyncTaskSpec with Matchers {
  ...
  private case object SummaryProbeTool extends Tool {
    ...
    val io: ToolIO[SummaryProbeInput, TextToolOutput] = ToolIO.derived[SummaryProbeInput, TextToolOutput]
    ...
    private def executeOutput(input: SummaryProbeInput, ctx: ToolContext): Task[TextToolOutput] =
