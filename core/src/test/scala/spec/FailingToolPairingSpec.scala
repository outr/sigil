...
class FailingToolPairingSpec extends AsyncWordSpec with AsyncTaskSpec with Matchers {
  ...
  private case object ErpTool extends Tool {
    ...
    val io: ToolIO[ErpInput, TextToolOutput] = ToolIO.derived[ErpInput, TextToolOutput]
    ...
    private def executeResult(input: ErpInput, ctx: ToolContext): Task[ToolResult[TextToolOutput]] =
