...
class IterationCapForcesSynthesisSpec extends AsyncWordSpec with AsyncTaskSpec with Matchers with org.scalatest.BeforeAndAfterAll {
      ...
      val emits: List[ProviderEvent] = if (isForcedRespondTurn) {
        ...
        List(
          ...
          ProviderEvent.toolCall(callId, RespondTool)(RespondInput(
            topicLabel = "Cap",
            topicSummary = "cap-hit synth",
            content = "synthesized from gathered context",
            endsTurn = true)),
   ...
   * without flaking.
   */
  private def eventsWhen(convId: Id[Conversation],
                         timeout: FiniteDuration = 30.seconds)(cond: List[sigil.event.Event] => Boolean): Task[List[sigil.event.Event]] = {
