...
class MalformedToolArgsDetectionSpec extends AnyWordSpec with Matchers {
  ...
  "Bug #171.B — degenerate-args detection" should {
    ...
    "carry the provider key so error attribution survives the throw" in {
      val acc = new ToolCallAccumulator(ToolRoster(Vector(RespondTool)), providerKey = "DigitalOcean")
      feed(acc, """[{}, {}]""")
      val ex = intercept[ProviderStreamException](acc.complete())
      ex.providerKey shouldBe "DigitalOcean"
    }

    "fall through to the Malformed path for non-degenerate parse failures" in {
      ...
      malformed.head.name shouldBe RespondTool.name.value
    }

    "fall through to the Malformed path for invalid JSON (not even a parseable array)" in {
      val acc = new ToolCallAccumulator(ToolRoster(Vector(RespondTool)), providerKey = "deepinfra")
      feed(acc, """{garbage""")
      val events = acc.complete()
      val malformed = events.collect { case ProviderEvent.ToolCallComplete(_, m: WireCall.Malformed) => m }
      malformed should not be empty
      malformed.head.rawArgs shouldBe fabric.Str("""{garbage""")
    }
