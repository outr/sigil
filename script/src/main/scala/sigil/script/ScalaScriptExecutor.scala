...
class ScalaScriptExecutor(classpathOverride: Option[String] = None) extends ScriptExecutor {
  ...
  private[script] lazy val engine: ScriptEngine = {
    ...
    // streams THEN. Scala 3.9.0's reporter writes to `System.out`/`System.err`
   ...
   */
  override def compile(source: String, bindingTypes: List[ScriptBinding]): Task[Either[List[CompileError], CompiledScript]] = Task {
    engine.synchronized {
